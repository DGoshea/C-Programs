/* Header file for the Binary Search Tree Class
    This file includes the declarations and implementations of the BST
    and BNode classes. With both being template of type T
*/
#include <iostream>
using namespace std;

template <typename T> //data will be of type T
class BST;
...
private:
    ...
    bool insertHelper(T e, BNode<T>*& subroot);
    void printInOrderHelper(BNode<T>* subroot)const;
    void printPreorderHelper(BNode<T>* subroot)const;
    void printPostorderHelper(BNode<T>* subroot)const;
    BNode<T>* Search(T e, BNode<T>* subroot);
    bool Remove(T e, BNode<T>*& subroot);
    void goToEnd(BNode<T>* subroot);
    void goToBeginning(BNode<T>* subroot);
    void goToNext(BNode<T>* subroot);
    void clearList(BNode<T>* subroot);
    bool emptyList(BNode<T>* subroot)const;
    BNode<T>* copyTree(const BNode<T>* subroot);
    BNode<T>* findLeftMost(BNode<T>* subroot);
    BNode<T>* findRightMost(BNode<T>* subrooot);
    void updateCursor(BNode<T>* subroot);
    friend class BNode<T>;
...
public:
    ...
    bool Remove(T e);
    void testPrint(); // Inorder traversal
    void atCursor();
    void goToBeginning();
    void goToEnd();
    void goToNext();
...
BNode<T>* BST<T>::Search(T e, BNode<T>* subroot){
    ...
    BNode<T>* found = Search(e, subroot->right);
    if(found == nullptr && subroot->data == e)
    {
        cursor = subroot;
        found = subroot;
    }
...
template <typename T>
void BST<T>::updateCursor(BNode<T>* subroot)
{
    if(subroot == cursor)
    {
        goToNext();
...
//Found the item to remove
    else if(e == subroot->data)
    {
        ...
        else
        {
            ...
            {
                temp = temp->left;
            //REPLACE SUBROOT->DATA WITH THE SUCCESSOR VALUE
                subroot->data = temp->data;
            //REMOVE THE SUCCESSOR FROM THE RIGHT SUBTREE OF SUBROOT
...
//Fourth Case: Recurse left to remove left
    ...
    {
        Remove(e, subroot->left);
        if(Remove(e, subroot->left) && cursor == subroot) //if return is true & cursor was deleted
        {
            updateCursor(subroot);
        }
...
//Fifth Case: Recuse Right to Remove Right
    ...
    {
        Remove(e, subroot->right);
        if(Remove(e, subroot->right) && cursor == subroot) //if return is true & cursor was deleted
        {
            updateCursor(subroot);
        }
...
*/
template <typename T>
BNode<T>* BST<T>::findRightMost(BNode<T>* subroot)
{
...
*/
template <typename T>
BNode<T>* BST<T>::findLeftMost(BNode<T>* subroot)
{
...
void BST<T>::goToNext()
{
    ...
    {
        cursor = findLeftMost(cursor->right);
    }
    else
    {
        BNode<T>* prev = nullptr;
        BNode<T>* current = root;
        while(current != nullptr && current != cursor)
        {
        ...
        if(current == cursor && prev != nullptr)
        {
            cursor = prev;
        }
...
void BST<T>::goToPrev()
{
    ...
    {
        cursor = findRightMost(cursor->left);
    }
    else
    {
        BNode<T>* prev = nullptr;
...
*/
template <typename T>
void BST<T>::clearList(BNode<T>* subroot)
{
    if(subroot != nullptr)
    {
